...
const getRotation = () => {
	if (!memory[window.scrollY]) {
		memory[window.scrollY] = (2 * Math.PI * window.scrollY) / window.innerHeight;
	}
...
const getScaleRotation = () => {
	...
	let res = ratio * Math.PI * (window.scrollY / (document.body.clientHeight - window.innerHeight - 500));
